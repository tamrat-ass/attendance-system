...
export async function POST(request: NextRequest) {
  try {
    ...
    // STEP 1: Eliminate duplicates within the uploaded file
    ...
    for (let i = 0; i < students.length; i++) {
      ...
      // Create identifier for duplicate detection (name + phone)
      const identifier = `${(student.full_name || '').toLowerCase().trim()}|${(student.phone || '').trim()}`;
      ...
      if (seenIdentifiers.has(identifier)) {
        duplicatesInFile.push({
          row: rowNum,
          student: student,
          reason: `Duplicate within file (same as another row)`
        });
        console.log(`⚠️ DUPLICATE IN FILE - Row ${rowNum}: ${student.full_name} (${student.phone})`);
      } else {
        seenIdentifiers.add(identifier);
        uniqueStudents.push({ ...student, originalRow: rowNum });
      }
    }

    console.log(`📊 File analysis: ${students.length} total, ${uniqueStudents.length} unique, ${duplicatesInFile.length} duplicates in file`);

    // STEP 2: Comprehensive validation of unique students only
    const validationErrors: string[] = [];
    
    for (let i = 0; i < uniqueStudents.length; i++) {
      const student = uniqueStudents[i];
      const rowNum = student.originalRow;
      ...
      if (student.gender) {
        ...
        if (!validGenders.includes(student.gender.trim())) {
          validationErrors.push(`Row ${rowNum}: Invalid gender "${student.gender}" (must be Male, Female, M, or F)`);
        }
      } else {
        // Gender is optional, will default to Male
        console.log(`Row ${rowNum}: No gender specified for ${student.full_name}, will default to Male`);
      }
    ...
    try {
      ...
      // Get all existing students to check for duplicates against database
      const [existingStudents]: any = await db.query(
        'SELECT full_name, phone FROM students'
      );
      ...
      // Process only unique students (duplicates within file already removed)
      for (const student of uniqueStudents) {
        ...
        if (existingSet.has(identifier)) {
          console.log('✗ SKIPPING duplicate student (exists in DB):', student.full_name, student.phone);
          skippedStudents.push({
            ...student,
            reason: 'Duplicate (same name and phone already exists in database)'
          });
          continue;
        }
      ...
      console.log('Students in database AFTER upload:', afterCount[0].count);
      console.log('Inserted:', insertedStudents.length, 'Skipped:', skippedStudents.length);
      console.log('Duplicates in file:', duplicatesInFile.length);
      console.log('=== BULK UPLOAD COMPLETE ===');

      return NextResponse.json({
        success: true,
        message: `Successfully processed ${students.length} students: ${insertedStudents.length} added, ${skippedStudents.length} skipped (DB duplicates), ${duplicatesInFile.length} duplicates removed from file`,
        data: insertedStudents,
        skipped: skippedStudents,
        duplicatesInFile: duplicatesInFile,
        summary: {
