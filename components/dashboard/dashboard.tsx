    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto p-3 sm:p-6">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className={`grid w-full mb-4 sm:mb-6 h-auto`} style={{ gridTemplateColumns: `repeat(${[permissions.can_mark_attendance, permissions.can_manage_students, permissions.can_view_reports, permissions.can_manage_users].filter(Boolean).length}, 1fr)` }}>
            {permissions.can_mark_attendance && (
              <TabsTrigger value="attendance" className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2 p-2 sm:p-3 text-xs sm:text-sm">
            ...
            {permissions.can_manage_users && (
              <TabsTrigger value="admin" className="flex flex-col sm:flex-row items-center gap-1 sm:gap-2 p-2 sm:p-3 text-xs sm:text-sm">
                <svg className="w-4 h-4 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
